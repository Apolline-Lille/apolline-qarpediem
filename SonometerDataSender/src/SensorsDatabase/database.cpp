...
"SELECT created FROM sensors.polls WHERE done=1 AND sent=0 LIMIT ";
...
static const char* get_sonometer_poll_list_sql =
"SELECT created FROM sonometer.sonometer_polls WHERE done=1 AND sent=0 LIMIT ";
vector<time_t> SensorsDatabase::get_sendable_sonometer_polls(size_t max){
	string sql = get_sonometer_poll_list_sql;
	sql += to_string(max);
	sql += ';';
	cout << sql << endl;	
...
"UPDATE sensors.polls SET sent=1 WHERE created=?;";
...
static const char* sonometer_poll_sent_sql =
"UPDATE sonometer.sonometer_polls SET sent=1 WHERE created=?;";
void SensorsDatabase::sonometer_poll_sent(time_t poll){
	Statement stmt(db, sonometer_poll_sent_sql);
	stmt.bind_value(1, (sqlite3_int64) poll);
	stmt.execute();
}

static const char* delete_old_polls_sql =
"DELETE FROM sensors.polls WHERE done = 0 OR created < ?;";
...
static const char* delete_old_sonometer_polls_sql =
"DELETE FROM sonometer.sonometer_polls WHERE done = 0 OR created < ?;";
void SensorsDatabase::delete_sonometer_old_polls(time_t older_than){
	Statement stmt(db, delete_old_sonometer_polls_sql);
	stmt.bind_value(1, (sqlite3_int64) older_than);
	stmt.execute();
}
