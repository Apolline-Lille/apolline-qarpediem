...
static void send_polls(vector<time_t> polls){
	...
	for(const time_t& poll_time : polls){
		...
		poll_t poll = database->get_sonometer_poll(poll_time);
	...
	if(send_json(root)){
		for(const time_t& poll_time : polls){
			database->sonometer_poll_sent(poll_time);
		}
	}		
...
void runtime_ip(void){
	...
	time_t now = time_monotonic();
	vector<time_t> sonometer_polls;
	int ready = digitalRead(GPIO_PIN);

	if((now - last_send >= send_interval) && ready == 0) {
		sonometer_polls = database->get_sendable_sonometer_polls(100);
		while(sonometer_polls.size() > 0){
			send_polls(sonometer_polls);
			sonometer_polls = database->get_sendable_sonometer_polls(100);
		}
		last_send = now;
	}
