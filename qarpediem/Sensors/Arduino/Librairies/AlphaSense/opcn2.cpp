...
void OPCN2::begin(uint8_t chip_select){
    ...
    // Set up SPI
    spi_settings = SPISettings(750000, MSBFIRST, SPI_MODE1);

    // Set the firmware version
    String info_str = this->read_information_string();
    info_str.replace(".", "");
    info_str.trim();
    _fv = info_str.substring(24, 26).toInt();
